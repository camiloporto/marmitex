...
public class LoginActivity extends AbstractMarmitexActivity implements MarmitexApplication.OnProfileCreatedCallback, MarmitexApplication.OnLoginFinishedCallback {
    ...
    private void newUser() {
        ...
        if (cancel) {
            ...
            getMarmitexApplication().createNewProfile(profile, this);
        }
    }
...
//            getMarmitexApplication().createNewProfile(p, this);
    ...
    public void onProfileCreated(ProfileCreatedEvent event) {
        ...
        if(event.isSuccess()) {
            Profile created = event.getCreatedProfile();
            getMarmitexApplication().login(created.getLogin(), created.getPass(), this);
        } else {
            Toast.makeText(
                    this, "Profile Creation Failed!: ", Toast.LENGTH_SHORT)
                    .show();
        }
    }
    ...
    public void onLoginFinished(LoginFinishedEvent event) {
        ...
        if(event.isSuccess()) {
            Intent i = new Intent(this, MarmitexActivity.class);
            startActivity(i);
        } else {
            Toast.makeText(
